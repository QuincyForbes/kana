...
13. **Smarter scheduler.** Leitner with fixed intervals (10min/1d/3d/7d/21d) treats all cards alike. FSRS or SM-2 with per-card ease would schedule better; at minimum add a 22nd+ day box so "known" keeps stretching.
...
32. **Accessibility pass.** aria-live on quiz card changes, focus trap review, contrast check on `--muted` text, larger touch targets on chart cells.
...
40. **Unify the two drill systems (bigger).** The guide's drill keeps its own score separate from the trainer's SRS. Long-term, guide drill misses should feed the trainer's per-character progress — one shared record of what you actually know.
